...
const useCart = () => {
  ...
  const addToCartMutation = useMutation({
    mutationFn: async ({ productId, quantity = 1, userId }: { productId: string; quantity?: number, userId: string }) => {
      const { data } = await api.post<{ cart: Cart }>("/cart", { productId, quantity, userId });
      return data.cart;
    },
